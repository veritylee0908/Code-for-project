...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  6 17:08:17 2026
...
if nriver == 1:
    ...
    stitle = 'River Eden at Sheepmount 2005 Climate uplift 61%'

# -------------------------
# Force numeric + build separate clean baseline and CC datasets
# -------------------------
for col in ['Time', 'Height', 'Flow', 'Time CC61', 'Height CC61', 'Flow CC61']:
    if col in Data.columns:
        Data[col] = pd.to_numeric(Data[col], errors='coerce')

Base = Data.dropna(subset=['Time', 'Height', 'Flow']).reset_index(drop=True)
CC   = Data.dropna(subset=['Time CC61', 'Height CC61', 'Flow CC61']).reset_index(drop=True)

# Climate uplift arrays (these drive FEV/box/fill)
time   = CC['Time CC61'].to_numpy(dtype=float)
height = CC['Height CC61'].to_numpy(dtype=float)
Flow   = CC['Flow CC61'].to_numpy(dtype=float)
