...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  5 21:30:06 2026
...
if nriver == 1:
    ...
    stitle = 'River Eden at Sheepmount 2005 Climate uplift 35%'

# -------------------------
# Force numeric + build separate clean baseline and CC datasets
# -------------------------
for col in ['Time', 'Height', 'Flow', 'Time CC35', 'Height CC35', 'Flow CC35']:
    if col in Data.columns:
        Data[col] = pd.to_numeric(Data[col], errors='coerce')

Base = Data.dropna(subset=['Time', 'Height', 'Flow']).reset_index(drop=True)
CC   = Data.dropna(subset=['Time CC35', 'Height CC35', 'Flow CC35']).reset_index(drop=True)

# Climate uplift arrays (these drive FEV/box/fill)
time   = CC['Time CC35'].to_numpy(dtype=float)
height = CC['Height CC35'].to_numpy(dtype=float)
Flow   = CC['Flow CC35'].to_numpy(dtype=float)
