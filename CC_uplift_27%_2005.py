...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  5 21:21:37 2026
...
if nriver == 1:
    ...
    stitle = 'River Eden at Sheepmount 2005 Climate uplift 27%'

# -------------------------
# Force numeric + build separate clean baseline and CC datasets
# -------------------------
for col in ['Time', 'Height', 'Flow', 'Time CC27', 'Height CC27', 'Flow CC27']:
    if col in Data.columns:
        Data[col] = pd.to_numeric(Data[col], errors='coerce')

Base = Data.dropna(subset=['Time', 'Height', 'Flow']).reset_index(drop=True)
CC   = Data.dropna(subset=['Time CC27', 'Height CC27', 'Flow CC27']).reset_index(drop=True)

# Climate uplift arrays (these drive FEV/box/fill)
time   = CC['Time CC27'].to_numpy(dtype=float)
height = CC['Height CC27'].to_numpy(dtype=float)
Flow   = CC['Flow CC27'].to_numpy(dtype=float)
